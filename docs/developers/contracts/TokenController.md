...
The `TokenController` contract is the **core token manager** within the protocol, governing **NXM minting, burning, and transfers**. It is **not meant to be directly integrated by users** but rather serves as an internal controller for **Governor, Registry, Cover, Ramm, StakingProducts, and Staking Pools**.
...
- ✅ Only protocol-approved contracts (e.g., Governor, Registry, Cover, Ramm, StakingProducts, StakingPool, Pool) can interact with it.
